  ...
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      connectedCallback() {
        ...
        const template = _currentDoc.querySelector(`#wcsaveTemplate`);
        ...
        this.input_presetName = this.shadowRoot.querySelector(
          "#input_presetName"
        );
      ...
      loadBanks() {
        ...
        Object.keys(this.banks).map((elem, index) => {
          this.nav_banks.insertAdjacentHTML(
            "beforeEnd",
            this.renderLink(this.banks[elem])
          );
        });

        Object.keys(buildInBank).map((elem, index) => {
          this.nav_banks.insertAdjacentHTML(
            "beforeEnd",
            this.renderLink(buildInBank[elem])
          );
        });
      ...
      listeners() {
        ...
        this.bt_loadPreset.onclick = e => {
          ...
          this.shadowRoot
            .querySelector("#div_dialog")
            .classList.toggle("hidden");
        };
        this.bt_openDialog.onclick = e => {
          if (
            !this.pedalboard.shadowRoot
      ...
      loadPresets() {
        ...
        this.allbanks = this.banks.concat(buildInBank);
        let bankSelected = this.allbanks.find(
          item => item._id == this.bankSelected
        );
        console.log(bankSelected);
        let _presets = bankSelected.presets;
        this.nav_presets.innerHTML = "";
        Object.keys(_presets).map((elem, index) => {
          this.nav_presets.insertAdjacentHTML(
            "beforeEnd",
            this.renderLink(_presets[elem])
          );
        });
      ...
      loadPreset() {
        ...
        this.plugs = bankSelected.presets.find(
          item => item._id == this.presetSelected
        ).plugs;
        this.plugsConnexions = bankSelected.presets.find(
          item => item._id == this.presetSelected
        ).connexions;
      ...
      loadPresetOnLoad() {
        let bankSelected = this.allbanks.find(
          item => item.label == this.resultBank
        );
        this.plugs = bankSelected.presets.find(
          item => item.label == this.resultPreset
        ).plugs;
        this.plugsConnexions = bankSelected.presets.find(
          item => item.label == this.resultPreset
        ).connexions;
      ...
      loadPluginStatesOnLoad(plugs) {
        for (let i = 0; i < plugs.length; i++) {
          console.log(plugs[i]);
          this.pedalboard
            .querySelector(`#${plugs[i].id}`)
            .setAttribute("params", JSON.stringify(plugs[i].settings));
        }
      }

      loadConnexionsOnLoad() {
        console.log(
          `-------------- loadConnexions (${this.plugsConnexions.length}) --------------`
        );
        for (let i = 0; i < this.plugsConnexions.length; i++) {
          ...
          if (this.plugsConnexions[i].out == "pedalIn2")
            this.pedalboard.changetomono();
          console.log(this.plugsConnexions[i].in.inputnumber);
          this.pedalboard.connect(
            this.pedalboard.querySelector(`#${this.plugsConnexions[i].out}`),
            this.pedalboard.querySelector(`#${this.plugsConnexions[i].in.id}`),
            this.plugsConnexions[i].in.inputnumber
          );
        }
      ...
      loadPluginStates() {
        for (let i = 0; i < this.plugs.length; i++) {
          ...
          this.pedalboard
            .querySelector(`#${this.plugs[i].id}`)
            .setAttribute("params", JSON.stringify(this.plugs[i].settings));
        }
      }
