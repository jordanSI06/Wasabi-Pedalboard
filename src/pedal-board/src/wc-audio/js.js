(function () {
  ...
  customElements.define('wc-audio', class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      Object.keys(urlAudio).map(
        (el, index) => {
          this.render(urlAudio[el]);
        }
      )
      this.shadowRoot.querySelector('#soundSample').setAttribute('src', './audio/BasketCase Greenday riff DI.mp3');
    ...
    // ----- METHODS: CUSTOM -----
    render(_data) {
      this.shadowRoot.querySelector('#select_audio').insertAdjacentHTML('beforeEnd', `<option value='./audio/${_data.path}'>${_data.label}</option>`);
    }
