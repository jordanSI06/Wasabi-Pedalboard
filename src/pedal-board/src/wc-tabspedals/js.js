  ...
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      attributeChangedCallback(name, oldValue, newValue) {
        ...
        if (newValue) {
          try {
            ...
            Object.keys(this._pedalList).map((el, index) => {
          ...
          try {
            this.shadowRoot
              .querySelector("#div_tabs")
              .querySelectorAll("label")
              .forEach(l => {
                l.onclick = e => this.listeners(e);
              });
            this.shadowRoot.querySelector(".bt_pinViewTabs").onclick = e =>
              this.listeners(e);
          } catch (error) {
            console.log(error);
          }
      ...
      connectedCallback() {
        ...
        const template = _currentDoc.querySelector(`#templateTabspedals`);
      ...
      render(_data) {
        ...
        if (div_container.querySelector(`#content-${_data.label}`) == null) {
          ...
          this.shadowRoot
            ...
            .insertAdjacentHTML(
              "beforeEnd",
              `<input id='tab-${_data.label}' type='radio' value='${_data.label}' name='input-tab' checked />`
            );
          this.shadowRoot
            .querySelector("#div_tabs")
            .insertAdjacentHTML(
              "beforeEnd",
              `<label for='tab-${_data.label}'>${_data.label} (${_data.contents.length})</label>`
            );
          div_container.insertAdjacentHTML(
            "beforeEnd",
            `<div id='content-${_data.label}' class='content hidden'></div>`
          );
        }
        for (let i = 0; i < _data.contents.length; i++) {
          div_container
            .querySelector(`#content-${_data.label}`)
            .insertAdjacentHTML(
              "beforeEnd",
              `<div class='pedals'><div id='${_data.contents[i].id}' class='pedal' draggable='true'></div><img src='${_data.contents[i].Thumbnail}'><span>${_data.contents[i].id}</span></div>`
            );
        }
      }
      ...
      listeners(event) {
        ...
        if (event.target.textContent === "") {
          if (this.currentOpenedTab.status === "opened") {
            this.shadowRoot
              .querySelector("#div_app")
              .classList.add("bottomTabs");
            this.currentOpenedTab.status = "closed";
            // change character ^ to reverse
            this.shadowRoot
              .querySelector("#icon_view")
              .setAttribute("icon", "icons:expand-less");
          } else {
            this.shadowRoot
              .querySelector("#div_app")
              .classList.remove("bottomTabs");
            this.currentOpenedTab.status = "opened";
            this.shadowRoot
              .querySelector("#icon_view")
              .setAttribute("icon", "icons:expand-more");
          }
          // display default content
          let checked = this.shadowRoot.querySelectorAll(
            "[name='input-tab']:checked"
          );
