(function () {
  ...
  customElements.define(`pedal-delay`, class extends PBPlugin(HTMLElement) {
    ...
    setKnobsListeners() {
      this.shadowRoot.querySelector('#knob1').addEventListener('change', (e) => this.effect.setFeedback(e.target.value / 100));
      this.shadowRoot.querySelector('#knob2').addEventListener('change', (e) => this.effect.setTime(e.target.value / 100));
      this.shadowRoot.querySelector('#knob3').addEventListener('change', (e) => this.effect.setMix(e.target.value / 100));
    }
