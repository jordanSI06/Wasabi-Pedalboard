(function () {
  ...
  customElements.define(`pedal-weirdphaser`, class extends PBPlugin(HTMLElement) {
    ...
    createAllInternNodes() {
        var wph = this;
        var host = this.shadowRoot.querySelector("#WPH");
        host.context = GlobalContext.context;
        host.load("https://wasabi.i3s.unice.fr/WebAudioPluginBank/Oliver-Larkin/WeirdPhaser/FaustWeirdPhaser.html").then(function (node) {
        // ppd.w = node._gui.properties.dataWidth.value;
        // ppd.h = node._gui.properties.dataHeight.value;  
        wph.soundNodeIn.connect(node);
        node.connect(wph.soundNodeOut);
        });
      }
  });
})();