(function () {
  ...
  customElements.define(`pedal-stereofreqshifter`, class extends PBPlugin(HTMLElement) {
    ...
    createAllInternNodes() {
        var sfs = this;
        var host = this.shadowRoot.querySelector("#SFS");
        host.context = GlobalContext.context;
        host.load("https://wasabi.i3s.unice.fr/WebAudioPluginBank/Oliver-Larkin/StereoFrequencyShifter/FaustStereoFreqShifter.html").then(function (node) {
        // ppd.w = node._gui.properties.dataWidth.value;
        // ppd.h = node._gui.properties.dataHeight.value;  
        sfs.soundNodeIn.connect(node);
        node.connect(sfs.soundNodeOut);
        });
      }
  });
})();